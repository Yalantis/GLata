...
class TestScene(sp: SceneParams = SceneParams()) : Scene(sp) {
    ...
    private fun createTopRow(rp: RendererParams) {
        ...
        rect1.mp.textureId = rp.managers.textureManager.add(Texture(rp, "ava_2").apply { isRepeating = true })
        rect1.animation = ScalingAnimation(0.8f, 1f, 1000f).apply {
            isInfinite = true
            interpolator = AccelerateDecelerateInterpolator()
        }
        rect1.mp.setShader(Shaders.DEFAULT_TEXTURE_SHADER)
        rect1.mp.transform.position.x = -1.1f
        rect1.mp.transform.position.y = rowHeight
        children.add(rect1)
        ...
        rect2.mp.textureId = rect1.mp.textureId
        ...
        rect3.mp.setShader(Shaders.DEFAULT_TEXTURE_SHADER)
        rect3.mp.transform.position.x = 1.1f
        rect3.mp.transform.position.y = rowHeight
        children.add(rect3)
    }
    ...
    private fun createMiddleRow(rp: RendererParams) {
        ...
        val rect2 = Rectangle.builder()
                ...
                .build()
        rect2.mp.setShader(Shaders.DEFAULT_COLOR_SHADER)
        ...
        val rect3 = Rectangle.builder()
                ...
                .build()
        rect3.mp.setShader(Shaders.DEFAULT_COLOR_SHADER)
        rect3.animation = AxisRotationAnimation(90f)
        rect3.mp.transform.position.x = 1.1f
        rect3.mp.transform.position.y = rowHeight
        children.add(rect3)
